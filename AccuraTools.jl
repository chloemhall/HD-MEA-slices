...
function STExChannel( channel::Vector{Float64}, parameters::Dict{String, Int64} )
    ...
    bit = parameters[ "bit" ];
    σ = parameters[ "cte" ];
     min_threshold = 10 ; # Set your minimum threshold value here
    index_real = [ ];
    q = 1;
    ...
    while J <= ( length( channel ) - Int( window - 1 ) )
        ...
        if !iszero( bin ) 
            ...
            # Add an if statement to check if thr is greater than the minimum threshold
            if abs(thr) > min_threshold
...
function FilteringMatrix( datactx::Matrix{Float64}, Variables::Dict{Any, Any} )
    ...
    for k = 1:size( datactx, 1 )
        ...
        MUA = remezFilter( Variables, channel);
        datafilt[ k, : ] = MUA;
    end
    return datafilt
end
...
function spikedetection(filtered_data, SamplingRate, organoid_channels)
    ...
    for i = 2: nChs # ignore channel 1 i.e. the calCh
